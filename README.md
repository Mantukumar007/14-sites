...
- Dashboard is hidden until the user logs in with Supabase Auth.
- Successful login events are stored in Supabase table `login_logs`.
...
```text
https://wkijgpnzttchqtwyynyx.supabase.co
```

Run the SQL in this file from Supabase SQL Editor:

```text
C:\Mantu\solar-sheet-webapp\supabase-schema.sql
```

That creates the `login_logs` table and RLS policies. User passwords are handled by Supabase Auth and are not stored in `login_logs`.
